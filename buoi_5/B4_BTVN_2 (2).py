...
with open("Downloads/tinh_trang.json", "w") as f:
...
ket_qua_bai_3 = [data_user for data_user in data_users  if data_user['job'] == "Luật sư" or data_user['job'] == "Kỹ sư phần mềm"]
...
ket_qua_bai_5 = [data_user['nguoi_than'] for data_user in data_users if data_user["name"] == "Phùng Văn BB"]
ket_qua["bai_5"] = ket_qua_bai_5
with open("tinh_trang.json", "w") as f:
    json.dump(ket_qua, f, indent=4)
# Hiển thị thông tin của người là đảng viên ( "dang_vien": 1)
ket_qua_bai_6 = [data_user for data_user in data_users if data_user["dang_vien"] == 1]
ket_qua["bai_6"] = ket_qua_bai_6
with open("tinh_trang.json", "w") as f:
    json.dump(ket_qua, f, indent=4)
# Hiển thị danh sách những người đã kết hôn ( có vợ hoặc có chồng hoặc có  con gái/ con trai).
                ...
                #         "da_ket_hon": 1
                #     }
ket_qua_bai_7 = [data_user for data_user in data_users]
for data_user in  data_users:
    da_ket_hon = data_user["nguoi_than"]
    for ng_than in da_ket_hon:
        if ng_than in ["chồng","vợ","con gái", "con trai"]:
            data_user["da_ket_hon"] = 1
            break
        else:
            data_user["chua_ket_hon"] = 0
# print(tinh_trang)
